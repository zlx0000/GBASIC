...
struct ParseTreeNode *parseLine(struct ParserContext *context)
{
	struct ParseTreeNode *node =
		(struct ParseTreeNode *)calloc(1, sizeof(struct ParseTreeNode));
	node->childCount = 0;
	node->type = ROOT;
...
int main(int argc, char **argv)
{
	...
	prog->ParseTreeNode = (struct ParseTreeNode **)calloc(
	...
	*context->tokens = token;
	...
	context->operandQueue =
		(struct OperandQueue *)calloc(1, sizeof(struct OperandQueue));
	context->tokenPtr = *context->tokens;
